...
export const Route = createFileRoute("/$slug/")({
	// 🔴 A LOADER, not an effect. The publishable key must exist before any other
	// request leaves the page — resolving it in a component would let `call()` fire
	// with an empty key and answer 401 on the first paint.
	loader: ({ params }) => bootstrapPortal(params.slug),
	errorComponent: () => (
		<main className="grid min-h-dvh place-items-center p-6">
			<div className="max-w-sm text-center">
				<h1 className="font-medium text-lg">No portal here</h1>
				<p className="mt-2 text-muted-foreground text-sm">
					This address does not belong to a published portal. Check the link the
					business gave you.
				</p>
			</div>
		</main>
