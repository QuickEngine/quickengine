 ...
 *
 * Recipes are internal recommendation intelligence, not a public onboarding directory.
 * Several businesses deliberately share a module and first-action configuration, allowing
 * natural-language onboarding to recognize a specific business without asking operators to
 * browse QuickDash's taxonomy.
 *
 * This package owns the catalog so Account recommendations and QuickDash first-value guidance
 * consume the same configuration without duplicating it across applications.
 */
export type BusinessRecipe = {
	 ...
	 */
	firstActions: readonly FirstActionDescriptor["id"][];
};
...
export const RECIPES: readonly BusinessRecipe[] = [
	// --- Retail & e-commerce -------------------------------------------------
	{
		id: "ecommerce",
		name: "Online store",
...
const haystack = (recipe: BusinessRecipe) =>
 ...
 */
export function searchRecipes(query: string): readonly BusinessRecipe[] {
...
export function groupRecipes(
	recipes: readonly BusinessRecipe[],
): Array<[RecipeCategory, BusinessRecipe[]]> {
	const groups = new Map<RecipeCategory, BusinessRecipe[]>();
...
export function findRecipe(id: string): BusinessRecipe | undefined {
