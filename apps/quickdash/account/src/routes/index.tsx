...
function WorkspacesPage() {
	...
	if (!active) return <main className="p-6">No organization was found.</main>;
	if (workspaces.isPending)
		return <main className="p-6">Loading workspaces…</main>;
	if (workspaces.isError) throw workspaces.error;

	const normalizedQuery = query.trim().toLowerCase();
	const visibleWorkspaces = workspaces.data.items.filter((workspace) => {
		if (statusFilter === "active" && workspace.archivedAt) return false;
		if (statusFilter === "archived" && !workspace.archivedAt) return false;
		return (
			!normalizedQuery ||
			[
				workspace.name,
				...
				<Popover>
					<PopoverTrigger asChild>
						<Button variant="outline" className="font-normal">
							<Funnel className="size-4" /> Filter
						</Button>
					</PopoverTrigger>
						...
						>
								...
								<div className="min-w-0">
									<h2 className="truncate font-medium text-foreground">
										{!workspace.archivedAt ? (
											<a
												href={`${quickDashUrl}/${workspace.id}`}
							...
							<div className="mt-6 grid grid-cols-2 gap-4 border-foreground/[0.06] border-t pt-4 text-sm">
								<div>
									...
									<p className="mt-1">{workspace.modules.length} enabled</p>
								</div>
								<div>
									<p className="text-muted-foreground text-xs">Created</p>
									<p className="mt-1">{createdDate(workspace.createdAt)}</p>
								</div>
							</div>
							{!workspace.archivedAt ? (
								...
								<Button disabled variant="outline" className="mt-5 w-full">
...
export const Route = createFileRoute("/")({ component: WorkspacesPage });
