...
// refresh-proof, and the tier cards read the same state. Annual is the default —
// we lead with it and let users switch to monthly.
//
// Replaces `nuqs`, which only works inside Next. TanStack Router already owns
// search params, so this is one hook rather than a dependency.
export const CYCLES = ["annual", "monthly"] as const;
export type Cycle = (typeof CYCLES)[number];

const isCycle = (value: unknown): value is Cycle =>
	typeof value === "string" && (CYCLES as readonly string[]).includes(value);

export function useBillingCycle(): [Cycle, (next: Cycle) => void] {
	const navigate = useNavigate();
	const search = useSearch({ strict: false }) as { billing?: unknown };
	// Anything unrecognised falls back rather than rendering a broken page — the
	// value comes from a URL a stranger can edit.
	const cycle: Cycle = isCycle(search.billing) ? search.billing : "annual";
