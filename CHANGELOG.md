...
- QuickEngine web front door (skeleton, pre-polish): a fixed frosted-glass header (logo mark + centered nav — Products / Pricing / Resources / Contact — with a hover-dim interaction and contiguous hit areas, plus Sign in + a Get Started pill), a locked-in responsive `.page-gutter` margin standard, and the self-hosted brand fonts (Clash Grotesk display + General Sans body via `next/font/local`, no external CDN flash). Real, on-brand **/sign-up** and **/sign-in** pages wired to the existing auth (email/password + Google/GitHub, with the 2FA challenge + recovery-code step handled on sign-in). These replace the raw `/dev` console as the product auth UI; the pages live in the web app and call the auth API. Post-signup currently lands on `/` (placeholder until the account dashboard exists).
