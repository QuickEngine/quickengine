...
function Page() {
	...
	// Account is the stable control plane and routes incomplete accounts into
	// onboarding.
	//
	// This was a server-side redirect under Next. Marketing is now a static SPA
	// with no server runtime, so the check runs in the browser instead — and
	// **fails open by design**: if the session lookup errors or is slow, the
	// marketing page renders. A sales page that blanks because auth is down is
	// far worse than showing a signed-in visitor one page they did not need.
