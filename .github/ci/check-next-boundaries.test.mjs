...
test("the repository matches the declared Next compatibility boundary", async () => {
	...
	assert.equal(result.serverActions.size, 18);
	assert.equal(result.routes.size, 4);
});
...
test("new and stale compatibility files require an explicit baseline change", () => {
	...
	assert.equal(SERVER_ACTION_BASELINE.size, 18);
	assert.equal(NEXT_ROUTE_BASELINE.size, 4);
});
